...
fn write_theme_section(out: &mut String) {
    ...
    writeln!(
        out,
        "Themes use 24-bit colors. Maki detects truecolor support from the \
         ...
         gets it wrong, set `MAKI_TRUECOLOR=1` to force truecolor or \
         `MAKI_TRUECOLOR=0` to force the fallback.\n"
    )
    .unwrap();
}
